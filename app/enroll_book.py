...
def app():
    ...
    if 'existing_books' not in st.session_state:
    ...
    if st.session_state.existing_books:
        existing_books_str = ', '.join(st.session_state.existing_books)
        st.info(f"'{existing_books_str}'은(는) 이미 내 책장에 있어요. :)")

    # 없는 책들에 대해 추가 여부 묻기
    if st.session_state.missing_books:
        missing_books_str = ', '.join(st.session_state.missing_books)
...
def add_books_to_shelf(books):
    st.session_state.missing_books = []  # 없는 책을 모으는 리스트
    st.session_state.existing_books = []  # 이미 있는 책을 모으는 리스트
